...
double intra_edge_exchange(
    ...
    double current_cost = problem_instance.get_distance(node_i, node_i_plus_1) + 
                          problem_instance.get_distance(node_j, node_j_plus_1);

    // Cost of edges to add (reversing segment from pos1+1 to pos2)
    // New edges are (i, j) and (i+1, j+1)
    double new_cost = problem_instance.get_distance(node_i, node_j) + 
                      problem_instance.get_distance(node_i_plus_1, node_j_plus_1);
