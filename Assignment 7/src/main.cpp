...
#include "algorithms/large_neighborhood_search.h"
...
void process_instance(const std::string& filename, const std::string& instance_name, json& results_json, int time_limit_ms) {
    ...
    for (const auto& variant : variants) {
        ...
        auto generate_solution = [&](int i, int& iterations) {
            ...
            std::vector<int> result = large_neighborhood_search(problem_instance, starting_solution, time_limit_ms, variant.use_ls, iterations);
            timer.end_stage();
            return result;
        };
