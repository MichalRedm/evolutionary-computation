...
std::vector<int> generate_greedy_cycle_solution(const std::vector<PointData>& data, const std::vector<std::vector<int>>& distance_matrix, int start_node_id) {
    ...
    // Find the best second node to form the initial 2-node cycle
    int best_second_node = -1;
    double min_initial_objective = std::numeric_limits<double>::max();

    for (int i = 0; i < total_nodes; ++i) {
        if (!visited[i]) {
            // Objective for a 2-node cycle: dist(A,B) + dist(B,A) + cost(A) + cost(B)
            double current_objective = 2.0 * distance_matrix[start_node_id][i] + data[start_node_id].cost + data[i].cost;
