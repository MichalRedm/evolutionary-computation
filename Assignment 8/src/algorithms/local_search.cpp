...
// Helper functions that were likely in neighborhood_utils.h
std::vector<int> get_intra_edge_exchange(int iterator, int size) {
    for (int i = 0; i < size - 1; ++i) {
        int num_pairs = size - 1 - i;
        if (iterator < num_pairs) {
            return {i, i + 1 + iterator};
        }
        iterator -= num_pairs;
    }
    return {0, 1};
...
std::vector<int> get_inter_node_exchange(const std::vector<int>& not_in_solution, const std::vector<int>& solution_pos, int iterator, int size) {
    int num_outside = not_in_solution.size();
    if (num_outside == 0) return {0, 0};
    int sol_idx = iterator / num_outside;
    int out_idx = iterator % num_outside;
    int pos_in_sol = solution_pos[sol_idx]; 
    int node_id = not_in_solution[out_idx];
    return {pos_in_sol, node_id};
}

void apply_change(NeighbourhoodType type, std::vector<int>& solution, const std::vector<int>& change, std::vector<int>& not_in_solution) {
    if (type == NeighbourhoodType::INTRA) {
        apply_intra_edge_exchange(solution, change[0], change[1]);
    } else {
        int pos_in_sol = change[0];
        int new_node = change[1];
        int old_node = solution[pos_in_sol];
        
        solution[pos_in_sol] = new_node;
        
        auto it = std::find(not_in_solution.begin(), not_in_solution.end(), new_node);
